            ...
            if is_symbol(s.op):
                if s.op == goal.op:
                    for i in range(len(s.args)):
                        if is_prop_symbol(s.args[i]) and not s.args[i].args and is_prop_symbol(goal.args[i])\
                            and not goal.args[i].args and s.args[i] != goal.args[i]:
                                break
                    else:
                        goal_clauses.append(s)
            else:
                antecedent, consequent = s.args
                if consequent.op == goal.op:
                    for i in range(len(consequent.args)):
                        if is_prop_symbol(consequent.args[i].op) and not consequent.args[i].args and is_prop_symbol(goal.args[i].op) \
                            and not goal.args[i].args and consequent.args[i] != goal.args[i]:
                          break
                    else:
                        goal_clauses.append(s)
        return goal_clauses
...
def fol_fc_ask(kb, alpha):
    ...
    def enum_subst(p):
        ...
        for assignment_list in itertools.permutations(kb_consts, r=len(query_vars)):
